...
These are the beacon proxy addresses that users interact with.
...
Shareland uses an **upgradeable beacon proxy pattern** (OpenZeppelin). The addresses above are the proxies that users interact with. Implementation contracts sit behind the beacon and can be upgraded without changing the proxy address.
